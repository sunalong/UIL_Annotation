...
public class LruDiscCache implements DiskCache {
	...
	private static final String TAG = "LruDiscCache";
	...
	public LruDiscCache(File cacheDir, File reserveCacheDir, FileNameGenerator fileNameGenerator, long cacheMaxSize,
		...
		Log.i(TAG,"LruDiscCache构造函数:"+"缓存路径："+cacheDir+" 保留的缓存路径："+reserveCacheDir+" 文件命名器："+fileNameGenerator+" 最大缓存："+cacheMaxSize+" 最大缓存数："+cacheMaxFileCount);
	...
	public File get(String imageUri) {
		...
		try {
			...
			if(snapshot !=null)
			Log.i(TAG,"LruDiscCache get:"+snapshot.getFile(0)+" 路径："+snapshot.getFile(0).getAbsolutePath());
			else
				Log.i(TAG,"LruDiscCache get:"+snapshot);
	...
	private String getKey(String imageUri) {
		String s = fileNameGenerator.generate(imageUri);
		Log.i(TAG,"使用文件名生成器获取key:"+s);
		return s;
	}
